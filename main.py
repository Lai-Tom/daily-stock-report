...
for index, item in enumerate(prompts):
    ...
    try:
        ...
        print(f"   ❌ 發生錯誤：{str(e)}")
        html_content += f"<div class='card'><h2>{item['title']}</h2><p class='error'>分析失敗，原因：{str(e)}</p></div>"

    # --- 安全防護：每跑完一個請求，休息 35 秒 ---
    if index < len(prompts) - 1:
        print("⏳ 為確保 Pro 模型連線穩定，系統冷卻 35 秒...")
        time.sleep(35)
